...
export default function EditWebsite() {
  ...
  const fetchWebsite = async () => {
    try {
      ...
      // Parse available services to populate form
      const availableServices = data.available_services || []
      const pricingAddons = data.pricing_addons || []
      
      // Extract service settings
      const guestPostService = availableServices.find((s: any) => s.type === 'guest_post')
      const linkPlacementService = availableServices.find((s: any) => s.type === 'link_placement')
      const sponsoredContentService = availableServices.find((s: any) => s.type === 'sponsored_content')
      
      // Extract addon settings
      const imageAddon = pricingAddons.find((a: any) => a.type === 'image_inclusion')
      const videoAddon = pricingAddons.find((a: any) => a.type === 'video_inclusion')
      const rushAddon = pricingAddons.find((a: any) => a.type === 'rush_delivery')

      // Extract content restrictions data
      const contentRestrictions = data.content_restrictions || {}
      const acceptedNiches = contentRestrictions.accepted_niches || []
      const nichePricing = contentRestrictions.niche_pricing || {}
      const linkTypes = contentRestrictions.link_types || {}

      // Build content niche acceptance object
      const contentNicheAcceptance: Record<string, boolean> = {}
      ...
      setFormData({
        websiteUrl: data.url || "",
        niche: data.primary_niche || "",
        domainAuthority: data.domain_authority?.toString() || "",
        monthlyVisits: data.monthly_visitors?.toString() || "",
        engagementRate: data.average_engagement_rate?.toString() || "",
        ...
        contactEmail: "",
        // Service acceptance
        acceptsLinkPlacement: !!linkPlacementService,
        basePriceLinkPlacement: linkPlacementService?.base_price?.toString() || "",
        acceptsGuestPost: !!guestPostService,
        basePriceGuestPost: guestPostService?.base_price?.toString() || "",
        acceptsSponsoredContent: !!sponsoredContentService,
        basePriceSponsoredContent: sponsoredContentService?.base_price?.toString() || "",
        // Link types - extract from content restrictions
        acceptsDofollow: linkTypes.dofollow || guestPostService?.dofollow_link || false,
        acceptsNofollow: linkTypes.nofollow || false,
        acceptsSponsored: linkTypes.sponsored || false,
        acceptsUgc: linkTypes.ugc || false,
        // Placement specifics
        acceptsHomepage: false,
        premiumPriceHomepage: "",
        acceptsExistingContent: false,
        basePriceExistingContent: "",
        acceptsNewContent: false,
        // Content niches - extract from content restrictions
        contentNicheAcceptance: contentNicheAcceptance,
        contentNichePricing: nichePricing,
        // Guest post specifics
        minWordCount: guestPostService?.min_words?.toString() || "",
        maxLinks: guestPostService?.max_links?.toString() || "",
        pricePerAdditionalWord: "",
        // Add-ons
        acceptsImageInclusion: !!imageAddon,
        priceImageInclusion: imageAddon?.price?.toString() || "",
        acceptsVideoInclusion: !!videoAddon,
        priceVideoInclusion: videoAddon?.price?.toString() || "",
        acceptsFastTurnaround: !!rushAddon,
        priceFastTurnaround: rushAddon?.price?.toString() || "",
        fastTurnaroundDays: "1",
        // Notes
        specialConditions: contentRestrictions.special_conditions || "",
        listingActive: data.status === 'active',
      })
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) {
      setError("You must be logged in to update a website")
      setIsLoading(false)
      return
    }
    ...
    try {
      ...
      if (error) {
        ...
        setError(error.message || "Failed to update website. Please try again.")
        setIsLoading(false)
        return
      }

      // Success! Redirect to websites list
      router.push('/publisher/websites?success=Website updated successfully')
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-slate-600">Update your website and service offerings</p>
          </div>
        </div>
